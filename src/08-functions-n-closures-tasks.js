...
function memoize(func) {
  ...
  return function a(...args) {
    const argst = JSON.stringify(...args);
    if (!(argst in cache)) {
      cache[argst] = func.apply(this, ...args);
    }
    return cache[argst];
  };
}
...
function logger(func, logFunc) {
  return function aa(...args) {
    const argsa = Array.from(...args);
    const argsString = argsa.map((arg) => JSON.stringify(arg)).join(', ');
